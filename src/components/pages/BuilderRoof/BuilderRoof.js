...
const BuilderRoof = () => {
  ...
  const scaleX = 87843.36; // google map to canvas
  const scaleY = 110947.2; // google map to canvas
  ...
  const initialBuildingData = useMemo(() => {
    ...
    tempInitialData.forEach((item, index) => {
      initialDataInfo.push({
        buildingIndex: index,
        roofHeight: 2,
        roofAngle: parseFloat((Math.atan(4 / (item.width * scaleX * 0.63)) * 180 / Math.PI).toFixed(1)),
        buildingHeight: 3,
        buildingDirection: 'east',
        ridgeDirection: 'direction_1',
        solarStyle: '1708x1134x30mm',
        solarDirection: 'horizontal',
        roofStyle: getRoofType(getRoofsInfo[index].southPosition),
        roofTexture: getRoofTexture(getRoofsInfo[index].roofType),
  ...
  useEffect(() => {
    ...
    initialBuildingData.map((buildingItem, buildingIndex) => {
      initialObstacleData.map((obstacleItem, obstacleIndex) => {
        if (
          buildingItem.position[0] - buildingItem.width / 2 < obstacleItem.position[0] &&
          buildingItem.position[0] + buildingItem.width / 2 > obstacleItem.position[0] &&
          buildingItem.position[2] - buildingItem.length / 2 < obstacleItem.position[2] &&
          buildingItem.position[2] + buildingItem.length / 2 > obstacleItem.position[2]
        ) {
          obstacleList.push({
    ...
    <div className={styles.mainWrapper}>
      <Row className={styles.content} >
        ...
        <Col xs={24} sm={14} md={16} className={styles.canvas}>
          <Compass />
          <Canvas id='main-canvas' camera={{ fov: '50', position: [0, 80, 0], }} shadows>
            {
              roofsData.map((item, index) => (<Building key={`building-${index}`} index={index} item={item} orbitCam={orbitCam} />))
            }
